...
export class RootConfig<T extends string>
  implements HasId<T>, HasChildren<ViewConfig<string>>, RepresentsRoutes<RootRoutePartial>
{
  constructor(public id: T, private views: ViewConfig<string>[]) {
    if (!views.length) {
      throw new Error(
        `Trying to create root ${id} without views. Root must have at least one view.`,
      );
    }
    views.forEach((views) => {
...
export function createRoot<
  ...
  C extends ViewConfig<any>,
  D extends ViewConfig<any>,
  E extends ViewConfig<any>,
  F extends ViewConfig<any>,
  G extends ViewConfig<any>,
>(
  id: T,
  panels: [A, B, C, D, E, F, G],
): AddChild<
  AddChild<AddChild<AddChild<AddChild<AddChild<AddChild<RootConfig<T>, A>, B>, C>, D>, E>, F>,
  G
>;
export function createRoot<
  T extends string,
...
>(
  id: T,
  panels: [A, B, C, D, E],
): AddChild<AddChild<AddChild<AddChild<AddChild<RootConfig<T>, A>, B>, C>, D>, E>;
export function createRoot<
  T extends string,
  A extends ViewConfig<any>,
  B extends ViewConfig<any>,
...
export function createRoot<T extends string, A extends ViewConfig<any>, B extends ViewConfig<any>>(
  id: T,
  views: [A, B],
): AddChild<AddChild<RootConfig<T>, A>, B>;
export function createRoot<T extends string, A extends ViewConfig<any>>(
  id: T,
  views: [A],
): AddChild<RootConfig<T>, A>;
